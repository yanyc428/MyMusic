...
public class OfflineSongIndex {
	 ...
	 */
	public OfflineSongIndex(int id, JFrame frame) {
...
//	
	...
	private void initialize(final int id, final JFrame frame) {
		...
		actionPanel.setLayout(new GridLayout(1, 1, 0, 0));
		...
		searchButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ArrayList<HashMap<String, String>> mapList= SongsTableActions.selectOffline(textField.getText());
				...
				for (final HashMap<String, String> item: mapList) {
					...
					if (id!=0){
						...
						if (LoveAMusic.check(id, Integer.parseInt(item.get("id")))){
						...
						loveSelectLabel.addMouseListener(new MouseAdapter() {
							...
							public void mouseClicked(MouseEvent e) {
								if (LoveAMusic.love(id, Integer.parseInt(item.get("id")))){
									loveSelectLabel.setIcon(iconLove);
								}else{
									loveSelectLabel.setIcon(iconNotLove);
								}
